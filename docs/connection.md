...
повторно использует локальный client key и меняет только путь `{file:...}`. Если
хотя бы одна проверка не проходит, автоматический переход прекращается. Не удаляйте
прежний каталог, пока новый doctor, запуск и пользовательский запрос не пройдут.
